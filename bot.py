...
async def BotzHub():
    async with user_bot:
        while True:
            ...
            await user_bot.edit_message(int(chnl_id), msg_id, "**@My_Test_Botz Bots Stats.**\n\n`Performing a periodic check...`")
            c = 0
            edit_text = "**List of all bots from @My_Test_Botz  and its online status:\n ✅= Up\n ❌= Down**\n\n"
            for bot in bots:
            ...
            edit_text +=f"\n**Last Checked:** \n`{t} - {day} {month} {year} [IST]`\n\n__Bots status are auto-updated every 5 hours__"
            await user_bot.edit_message(int(chnl_id), msg_id, edit_text)
